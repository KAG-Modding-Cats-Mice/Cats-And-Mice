...
void onRender(CRules@ this)
{
	...
	if (p.getBlob() is null && this.exists(propname))
	{
		...
		if (spawn != 255)
		{
			...
			{
				GUI::DrawText(getTranslatedString("                      Wait until you respawn!") , Vec2f(getScreenWidth() / 2 - 180, getScreenHeight() / 3 + Maths::Sin(getGameTime() / 3.0f) * 5.0f), SColor(255, 255, 255, 55));
			}
			else
			{
